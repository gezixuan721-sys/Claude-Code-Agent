...
class CcaTuiApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]Claude-Code-Agent[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]Claude-Code-Agent[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]Claude-Code-Agent[/bold]  [red]subscribe error: {e}[/red]")
            finally:
